...
export function Roulette() {
  ...
  const spin = async () => {
    ...
    setTimeout(async () => {
      ...
      if (bets[winningNumber.toString()])
        winnings += bets[winningNumber.toString()] * 36;
      ...
      if (winningNumber !== 0) {
        ...
        if (winningNumber % 2 === 0 && bets["even"])
          winnings += bets["even"] * 2;
      }
      if (winningNumber >= 1 && winningNumber <= 18 && bets["1-18"])
        winnings += bets["1-18"] * 2;
      if (winningNumber >= 19 && winningNumber <= 36 && bets["19-36"])
        winnings += bets["19-36"] * 2;
      if (winningNumber >= 1 && winningNumber <= 12 && bets["doz1"])
        winnings += bets["doz1"] * 3;
      if (winningNumber >= 13 && winningNumber <= 24 && bets["doz2"])
        winnings += bets["doz2"] * 3;
      if (winningNumber >= 25 && winningNumber <= 36 && bets["doz3"])
        winnings += bets["doz3"] * 3;

      if (winningNumber !== 0) {
        if (winningNumber % 3 === 0 && bets["col3"])
          winnings += bets["col3"] * 3;
        if (winningNumber % 3 === 2 && bets["col2"])
          winnings += bets["col2"] * 3;
        if (winningNumber % 3 === 1 && bets["col1"])
          winnings += bets["col1"] * 3;
      }
  ...
  const getCellColor = (n: number | string) => {
    if (n === 0 || n === "0")
      return "bg-[#43b359] hover:bg-[#348f46] text-white border-transparent";
    if (typeof n === "number" && getColor(n) === "red")
      return "bg-[#ed4163] hover:bg-[#c23652] text-white border-transparent";
    if (typeof n === "number" && getColor(n) === "black")
  ...
  const betChip = (amt: number) => (
    <div className="absolute inset-0 flex items-center justify-center z-10 pointer-events-none">
      <div className="w-5 h-5 rounded-full border border-dashed border-[#ffca28] bg-[#ff8f00] flex items-center justify-center text-[7px] font-black shadow-lg">
        <span className="drop-shadow bg-black/30 px-0.5 rounded text-white">
          {amt < 1 ? "•" : amt >= 1000 ? amt / 1000 + "K" : amt}
        </span>
      </div>
    </div>
  );
          ...
          <div className="flex flex-col gap-1 mt-2">
            <div className="flex justify-between items-center text-xs font-semibold text-[#8b9ba5] opacity-80">
              ...
              <span className="text-white flex items-center gap-1 font-mono">
                {betAmount.toFixed(8)}{" "}
                <span className="text-emerald-500 bg-emerald-500/20 w-3.5 h-3.5 rounded-full flex items-center justify-center text-[8px] leading-none">
                  T
                </span>
              </span>
            </div>
              ...
              <div className="flex-1 flex justify-center items-center gap-1.5">
                {[1, 10, 100, 1000, 10000].map((v) => (
                  <button
                    ...
                    className={cn(
                      "w-7 h-7 rounded-full border shadow-sm flex items-center justify-center font-bold text-[9px] transition-transform hover:scale-110 relative",
                      betAmount === v
                        ? "border-yellow-300 scale-110 z-10 text-yellow-900"
                        : "border-yellow-700 opacity-80 text-yellow-900",
                      v === 10000
                        ? "opacity-30 cursor-not-allowed text-transparent"
                        : "",
                    )}
                    style={{
            ...
            >
              ...
              >
                ...
                {WHEEL_NUMBERS.map((n, i) => {
                  ...
                  const radStart =
                    (startAngle - 90 - sliceAngle / 2) * (Math.PI / 180);
                  const radEnd =
                    (startAngle - 90 + sliceAngle / 2) * (Math.PI / 180);
                ...
                <path
                  d="M 50 34 L 50 66 M 34 50 L 66 50"
                  stroke="#ca8a04"
                  strokeWidth="1.5"
                  strokeLinecap="round"
                />
                <path
                  d="M 44 44 L 56 56 M 44 56 L 56 44"
                  stroke="#ca8a04"
                  strokeWidth="1.5"
                  strokeLinecap="round"
                />
          ...
          <div className="w-full max-w-[700px] flex flex-col gap-1.5 selection:bg-transparent -mt-2">
            ...
            <div className="flex w-full gap-1.5 min-h-[120px] md:min-h-[140px]">
              ...
              <button
                ...
                className={cn(
                  "w-[7.14%] rounded-sm text-lg font-bold relative overflow-hidden transition-colors flex items-center justify-center",
                  getCellColor("0"),
                )}
              >
                0{bets["0"] > 0 && betChip(bets["0"])}
              </button>
              ...
              <div className="w-[85.71%] grid grid-cols-12 grid-rows-3 gap-1.5">
                ...
                {[3, 6, 9, 12, 15, 18, 21, 24, 27, 30, 33, 36].map((n) => (
                  <button
                    ...
                    className={cn(
                      "rounded-sm flex items-center justify-center font-bold relative overflow-hidden transition-colors",
                      getCellColor(n),
                    )}
                  >
                    {n}
              ...
              <div className="w-[7.14%] grid grid-rows-3 gap-1.5">
                <button
                  onClick={() => handlePlaceBet("col3")}
                  className="rounded-sm bg-bg-inner hover:bg-border-subtle text-white text-xs font-bold relative flex items-center justify-center transition-colors"
                >
                  2:1 {bets["col3"] > 0 && betChip(bets["col3"])}
                </button>
                <button
                  onClick={() => handlePlaceBet("col2")}
                  className="rounded-sm bg-bg-inner hover:bg-border-subtle text-white text-xs font-bold relative flex items-center justify-center transition-colors"
                >
                  2:1 {bets["col2"] > 0 && betChip(bets["col2"])}
                </button>
                <button
                  onClick={() => handlePlaceBet("col1")}
                  className="rounded-sm bg-bg-inner hover:bg-border-subtle text-white text-xs font-bold relative flex items-center justify-center transition-colors"
                >
            ...
            <div className="flex w-full gap-1.5 h-[45px] pr-[7.14%] pl-[7.14%]">
              <button
                onClick={() => handlePlaceBet("doz1")}
                className="flex-1 rounded-sm bg-bg-inner hover:bg-border-subtle text-white text-sm font-bold relative overflow-hidden transition-colors"
              >
                1 to 12 {bets["doz1"] > 0 && betChip(bets["doz1"])}
              </button>
              <button
                onClick={() => handlePlaceBet("doz2")}
                className="flex-1 rounded-sm bg-bg-inner hover:bg-border-subtle text-white text-sm font-bold relative overflow-hidden transition-colors"
              >
                13 to 24 {bets["doz2"] > 0 && betChip(bets["doz2"])}
              </button>
              <button
                onClick={() => handlePlaceBet("doz3")}
                className="flex-1 rounded-sm bg-bg-inner hover:bg-border-subtle text-white text-sm font-bold relative overflow-hidden transition-colors"
              >
            ...
            <div className="flex w-full gap-1.5 h-[45px] pr-[7.14%] pl-[7.14%]">
              ...
              <button
                ...
                className="flex-1 rounded-sm bg-[#ed4163] hover:opacity-80 text-white relative overflow-hidden transition-colors"
              >
                {bets["red"] > 0 && betChip(bets["red"])}
              </button>
              <button
                onClick={() => handlePlaceBet("black")}
                className="flex-1 rounded-sm bg-[#213743] hover:bg-[#2c4755] text-white relative overflow-hidden transition-colors"
              >
