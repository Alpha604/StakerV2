...
export function Chicken() {
  ...
  const nextMultiplier = calculateMultiplier(chickensFound + 1, 25 - bonesCount);
  ...
  return (
    <div className="flex flex-col md:flex-row gap-4 max-w-[1200px] mx-auto p-4 md:p-8 min-h-[calc(100vh-80px)]">
      ...
      <div className="w-full md:w-80 bg-bg-panel border border-border-subtle rounded-t-xl md:rounded-l-xl md:rounded-tr-none flex flex-col h-fit order-2 md:order-1 z-10 p-4 gap-4">
         ...
         <div className="flex flex-col gap-2">
            <label className="text-sm font-bold text-text-secondary uppercase tracking-wider">Bones (Os)</label>
            <select
                value={bonesCount}
         ...
         {!isPlaying ? (
             ...
             >
                ...
                {chickensFound > 0 && <span className="text-sm leading-tight opacity-80">€{(betAmount * currentMultiplier).toFixed(2)}</span>}
             </button>
         )}
      </div>
      ...
      <div className="flex-1 bg-[#0f172a] rounded-b-xl md:rounded-r-xl border border-border-subtle relative overflow-hidden order-1 md:order-2 p-4 flex flex-col items-center justify-center min-h-[500px]">
              ...
              {grid.map((cell, i) => (
                  <button
                     ...
                     className={cn(
                        ...
                        cell === 'HIDDEN' ? "bg-[#213743] hover:bg-[#2c4755] cursor-pointer" : "bg-[#111c22] pointer-events-none opacity-80 border border-border-medium/30",
                        !isPlaying && cell === 'HIDDEN' && "opacity-50 cursor-not-allowed" // Initial state before playing
                     )}
                  >
                        {cell === 'HIDDEN' && isPlaying && <div className="absolute inset-x-0 bottom-0 border-b-4 border-[#1a2c38] pointer-events-none rounded-b-md"></div>}

                        {cell === 'CHICKEN' && (
                             <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="text-4xl filter drop-shadow-[0_0_8px_rgba(255,255,255,0.4)]">🍗</motion.div>
                        )}
                        {cell === 'BONE' && (
                             <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="text-4xl text-red-500 filter drop-shadow-[0_0_8px_rgba(239,68,68,0.4)]"><Skull size={40} /></motion.div>
                        )}
                  </button>
              ))}
          </div>
          ...
          <div className="absolute top-4 right-4 bg-bg-panel/80 backdrop-blur px-4 py-2 rounded-lg border border-border-medium shadow-lg font-mono font-bold text-white flex gap-4">
               <div className="flex flex-col items-center">
                   <span className="text-xs text-text-secondary uppercase">Suivant</span>
