...
export function Blackjack() {
  ...
  const [gameStage, setGameStage] = useState<'IDLE' | 'PLAYING' | 'DEALER_TURN' | 'ENDED'>('IDLE');
  ...
  const handleEnd = (finalPlayerHand: number[], finalDealerHand: number[], isBlackjack = false) => {
     setGameStage('ENDED');
    ...
    <div className="flex flex-col md:flex-row gap-4 max-w-[1200px] mx-auto p-4 md:p-8 min-h-[calc(100vh-80px)]">
      ...
      <div className="w-full md:w-80 bg-bg-panel border border-border-subtle rounded-t-xl md:rounded-l-xl md:rounded-tr-none flex flex-col h-fit order-2 md:order-1 z-10 p-4 gap-4">
         ...
         <div className="flex flex-col gap-2">
            ...
            <div className="relative bg-bg-inner border border-border-medium rounded flex items-center hover:border-text-secondary transition-colors focus-within:border-accent">
               ...
               <div className="pr-1 flex gap-1">
                   <button onClick={() => setBetAmount(b => b/2)} className="bg-bg-panel hover:bg-border-subtle p-1.5 rounded font-bold text-xs">1/2</button>
                   <button onClick={() => setBetAmount(b => b*2)} className="bg-bg-panel hover:bg-border-subtle p-1.5 rounded font-bold text-xs">2x</button>
               </div>
            </div>
         </div>
         ...
         {gameStage === 'IDLE' || gameStage === 'ENDED' ? (
            ...
            <div className="grid grid-cols-2 gap-2 mt-2">
               <button onClick={hit} className="w-full bg-[#1475e1] hover:bg-[#1b80f0] text-white py-3 font-bold rounded transition-colors shadow">Tirer</button>
               <button onClick={() => stand()} className="w-full bg-[#ed4163] hover:bg-[#f05273] text-white py-3 font-bold rounded transition-colors shadow">Rester</button>
            </div>
         )}
      </div>
      ...
      <div className="flex-1 bg-[#1a2c38] rounded-b-xl md:rounded-r-xl border border-border-subtle relative overflow-hidden order-1 md:order-2 p-8 flex flex-col justify-between min-h-[500px]">
         ...
         <div className="flex flex-col items-center">
            <span className="text-text-secondary font-bold mb-4">Croupier {gameStage === 'PLAYING' && dealerHand.length > 0 ? `(?)` : `(${getHandTotal(dealerHand)})`}</span>
                ...
                {dealerHand.map((card, i) => (
                    ...
                    >
                        {gameStage === 'PLAYING' && i === 1 ? (
                            ...
                            <span className={cn("text-3xl font-black", card === 11 ? 'text-red-500' : 'text-black')}>{card === 11 ? 'A' : card}</span>
                        )}
                    </motion.div>
                ))}
            </div>
         ...
         <div className="flex flex-col items-center">
             ...
             <span className="text-white font-bold mt-4 text-xl">Vous ({getHandTotal(playerHand)})</span>
         </div>
      </div>
    </div>
  );
