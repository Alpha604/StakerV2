...
import { X, ArrowRightLeft, Wallet, Vault, CreditCard, Bitcoin, Landmark, ArrowDownToLine, ArrowUpFromLine } from 'lucide-react';
import { motion } from 'motion/react';
import { cn } from '../lib/utils';

export function WalletModal({ onClose }: { onClose: () => void }) {
  const { user, balance, vault, transferToVault, transferFromVault, addBalance, subtractBalance } = useUser();
        ...
        <div className="p-6 flex flex-col gap-6">
          ...
          <div className="flex gap-4">
             ...
             <div className="flex-1 bg-bg-base border border-border-medium shadow-inner rounded-xl p-4 flex flex-col justify-center items-center relative overflow-hidden">
                ...
                <span className="text-xl font-bold font-mono text-indigo-400">${vault.toFixed(2)}</span>
             </div>
          </div>
          ...
          <div className="bg-bg-base border border-border-medium rounded-xl p-1 grid grid-cols-4 gap-1">
             <button
               onClick={() => setTab('buy')}
               className={cn("py-2 px-1 text-xs font-bold rounded-lg transition-all flex flex-col items-center justify-center gap-1", tab === 'buy' ? "bg-bg-panel text-white shadow border border-border-medium" : "text-text-secondary hover:text-white")}
             >
               <ArrowDownToLine size={14} className="text-green-500"/> Acheter
             </button>
             <button
               onClick={() => setTab('cashout')}
               className={cn("py-2 px-1 text-xs font-bold rounded-lg transition-all flex flex-col items-center justify-center gap-1", tab === 'cashout' ? "bg-bg-panel text-white shadow border border-border-medium" : "text-text-secondary hover:text-white")}
             >
               <ArrowUpFromLine size={14} className="text-red-500"/> Retirer
             </button>
             <button
               onClick={() => setTab('vault_in')}
               className={cn("py-2 px-1 text-xs font-bold rounded-lg transition-all flex flex-col items-center justify-center gap-1", tab === 'vault_in' ? "bg-bg-panel text-white shadow border border-border-medium" : "text-text-secondary hover:text-white")}
             >
               <Vault size={14} className="text-indigo-400"/> Vers Vault
             </button>
             <button
               onClick={() => setTab('vault_out')}
               className={cn("py-2 px-1 text-xs font-bold rounded-lg transition-all flex flex-col items-center justify-center gap-1", tab === 'vault_out' ? "bg-bg-panel text-white shadow border border-border-medium" : "text-text-secondary hover:text-white")}
             >
          ...
          <div className="flex flex-col gap-4">
             <div className="flex flex-col gap-1.5">
               <div className="flex justify-between items-center">
                 ...
                 {(tab === 'vault_in' || tab === 'cashout') && <span className="text-xs text-text-secondary font-mono">Max: ${balance.toFixed(2)}</span>}
                 {tab === 'vault_out' && <span className="text-xs text-text-secondary font-mono">Max: ${vault.toFixed(2)}</span>}
               </div>
                 ...
                 <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                   {['10', '50', tab === 'buy' ? '100' : 'Max'].map(m => (
                     <button 
                       key={m}
                       onClick={() => {
                          if (m === 'Max') setAmount(tab === 'vault_out' ? String(vault || 0) : String(balance));
                          else setAmount(m);
                       }}
               ...
               {successMsg && <span className="text-xs text-[#00e676] font-semibold mt-1">{successMsg}</span>}
             </div>
             ...
             <button 
               ...
               className={cn(
                 ...
                 tab === 'buy' ? "bg-[#00e676] hover:bg-[#00c853] text-[#0a1e12] shadow-[0_0_15px_rgba(0,230,118,0.2)]" : 
                 tab === 'cashout' ? "bg-bg-inner hover:bg-border-subtle border border-border-medium text-white" :
                 tab === 'vault_out' ? "bg-indigo-600 hover:bg-indigo-500 text-white" : 
             ...
             >
               {loading ? (
                 ...
                 tab === 'buy' ? 'Acheter depuis le compte' : 
                 tab === 'cashout' ? 'Encaisser (Retirer sur compte)' : 
                 tab === 'vault_out' ? 'Transférer vers Solde de Jeu' : 
