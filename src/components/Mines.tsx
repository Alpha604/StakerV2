...
const playSound = (type: "gem" | "bomb" | "cashout") => {
  try {
    ...
    if (type === "gem") {
      ...
      osc.type = "sine";
      osc.frequency.setValueAtTime(196.0, audioCtx.currentTime); // G3
      ...
      <div className="flex flex-col md:flex-row gap-4 max-w-[1200px] mx-auto p-4 md:p-8 min-h-[calc(100vh-80px)]">
            ...
            <div className="flex flex-col gap-1.5 relative z-10">
              ...
              <div className="flex items-center bg-bg-inner border border-border-medium rounded shadow-inner h-10">
                <span className="pl-3 absolute">
                  <span className="text-emerald-500 font-bold font-mono text-sm leading-none bg-emerald-500/20 w-4 h-4 flex items-center justify-center rounded-full">
                    T
                  </span>
                </span>
                <input
        ...
        <div className="flex-1 bg-bg-inner rounded-b-xl md:rounded-r-xl md:rounded-bl-none flex flex-col items-center justify-center order-1 md:order-2 border border-border-subtle p-8 md:p-16 relative">
          ...
          {isPlaying && (
            <div className="absolute top-4 left-1/2 -translate-x-1/2 flex items-center gap-2">
              <span className="text-white text-sm font-bold bg-border-medium px-4 py-1.5 rounded-full shadow">
                Prochain :{" "}
                {calculateMultiplier(minesCount, revealedCount + 1).toFixed(2)}×
              </span>
            </div>
          )}
          ...
          <div className="grid grid-cols-5 gap-2 md:gap-3 w-full max-w-[500px] aspect-square">
            {grid.map((cell, i) => {
              const isLatest =
                isPlaying && cell !== "hidden" && revealedCount > 0; // rough approximation for animation trigger
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    cell === "hidden"
                      ? "bg-[#213743] hover:-translate-y-[2px] hover:bg-[#2c4755] cursor-pointer shadow-[0_4px_0_#15242d] active:shadow-[0_0px_0_#15242d] active:translate-y-1"
                      : "bg-[#0f172a] shadow-none",
                    !isPlaying &&
                      cell === "hidden" &&
                      "opacity-80 hover:-translate-y-0 cursor-default shadow-[0_4px_0_#15242d]",
                  )}
                >
                  <div
                    className={cn(
                    ...
                    <AnimatePresence>
                      {cell === "gem" && (
                        ...
                        >
                          ...
                          >
                            <path
                              d="M256 0L493.597 131.6L256 512L18.4026 131.6L256 0Z"
                              fill="#00E701"
                            />
                            <path
                              d="M256 0V512L18.4026 131.6L256 0Z"
                              fill="#00C001"
                            />
                            <path
                              d="M256 0L493.597 131.6L256 186.2V0Z"
                              fill="#1FFF20"
                              opacity="0.6"
                            />
                            <path
                              d="M256 0V186.2L18.4026 131.6L256 0Z"
                              fill="#00FF01"
                              opacity="0.4"
                            />
                          </svg>
                      ...
                      {cell === "bomb" && (
                        ...
                        >
                          <Bomb
                            size={48}
                            className="text-[#ed4163] drop-shadow-[0_0_15px_rgba(237,65,99,0.5)]"
                            fill="#ed4163"
                          />
                        </motion.div>
                      )}
