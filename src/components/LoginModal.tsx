  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (success) {
        ...
        setError(isRegister ? 'Ce nom d\'utilisateur est déjà pris' : 'Identifiants incorrects. Veuillez réessayer.');
      }
    } catch (err) {
        ...
        <div className="flex justify-between items-center p-6 border-b border-border-medium bg-bg-base/50 relative">
          ...
          <div className="absolute bottom-0 left-6 right-16 flex gap-4">
              <div className="w-1/2 flex justify-center pb-2 relative">
                 {isRegister ? <div className="absolute -bottom-px w-full h-0.5 bg-transparent" /> : <motion.div layoutId="login-tab" className="absolute -bottom-px w-full h-0.5 bg-accent" />}
              </div>
              <div className="w-1/2 flex justify-center pb-2 relative">
                 {!isRegister ? <div className="absolute -bottom-px w-full h-0.5 bg-transparent" /> : <motion.div layoutId="login-tab" className="absolute -bottom-px w-full h-0.5 bg-accent" />}
              </div>
          ...
          <button onClick={onClose} className="absolute top-4 right-4 text-text-secondary hover:text-white transition-colors bg-bg-inner hover:bg-border-subtle p-1 rounded-full">
            <X size={20} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-5">
             <div className="flex flex-col gap-2 relative">
               <label className="text-sm font-bold text-text-secondary flex items-center gap-2">Nom d'utilisateur</label>
               <div className="relative flex items-center bg-bg-inner rounded-lg border border-border-medium hover:border-text-secondary focus-within:border-accent focus-within:ring-1 focus-within:ring-accent transition-all">
                  <span className="pl-4 text-text-secondary"><User size={18} /></span>
                  <input 
                     type="text" 
                     value={username}
             ...
             <div className="flex flex-col gap-2 relative">
               <label className="text-sm font-bold text-text-secondary flex items-center gap-2">Mot de passe</label>
               <div className="relative flex items-center bg-bg-inner rounded-lg border border-border-medium hover:border-text-secondary focus-within:border-accent focus-within:ring-1 focus-within:ring-accent transition-all">
                  <span className="pl-4 text-text-secondary"><Lock size={18} /></span>
                  <input 
                     type="password" 
                     value={password}
               ...
               {!isRegister && <button type="button" className="text-xs text-text-secondary hover:text-white mt-1 self-end transition-colors">Mot de passe oublié ?</button>}
             </div>
             ...
             {isRegister && (
               <div className="flex items-start gap-2 bg-accent/10 border border-accent/20 p-3 rounded-lg">
                 ...
                 <p className="text-xs text-text-secondary">En créant un compte, vous acceptez nos conditions d'utilisations. Votre progression sera sauvegardée via votre navigateur.</p>
               </div>
             )}
          </div>
