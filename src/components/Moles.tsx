...
export function Moles() {
  ...
  const calculateMultiplier = (cleared: number, totalMoles: number) => {
    ...
    for (let i = 0; i < cleared; i++) {
        // Prob of hitting a mole: (totalMoles - i) / (totalTiles - i)
        prob *= (totalMoles - i) / (totalTiles - i);
    }
  ...
  const handleClick = (idx: number) => {
     ...
     if (!newBoard[idx].isMole) {
         ...
         if (newRevealedCount === molesCount) {
             // Found ALL moles! Won the game!
      ...
      <div className="w-full grid grid-cols-1 lg:grid-cols-[320px_1fr] bg-bg-panel rounded-lg overflow-hidden shadow-2xl min-h-[600px]">
        ...
        <div className="bg-bg-panel lg:bg-[#213743] p-4 flex flex-col gap-5 border-b lg:border-b-0 lg:border-r border-border-medium z-10">
          ...
          <div className="flex flex-col gap-4">
            ...
            {/* Moles Selection */}
            <div className="flex flex-col gap-1.5">
                <label className="text-text-secondary text-xs font-bold px-1"> Taupes à trouver </label>
                <select 
                  value={molesCount}
        ...
        <div className="bg-[#0f212e] relative p-4 lg:p-8 flex flex-col items-center justify-center min-h-[400px]">
            ...
            <div className="grid grid-cols-5 gap-2 sm:gap-3 lg:gap-4 max-w-[500px] w-full">
                {board.map((tile, idx) => {
                    const isWinningTile = tile.revealed && tile.isMole;
                    const isLosingTile = tile.revealed && !tile.isMole && gameOver;
                    ...
                    return (
                        <div 
                            ...
                            className={cn(
                                "aspect-square rounded-full transition-all duration-300 relative group flex items-center justify-center",
                                tile.revealed 
                                  ? "bg-[#1e2d36] shadow-inner" 
                                  : isPlaying 
                        ...
                        >
                            <div className={cn(
                                ...
                                {tile.revealed && (
                                   tile.isMole 
                                     ? <span className="text-3xl sm:text-4xl md:text-5xl filter drop-shadow opacity-100">
                                            🐭
                                       </span> 
                                     : <span className="text-3xl sm:text-4xl md:text-5xl filter drop-shadow relative opacity-90">
                            ...
                            {/* Hole cover if not revealed */}
                            {!tile.revealed && (
                                <span className="text-2xl sm:text-3xl filter drop-shadow opacity-50 group-hover:opacity-80 transition-opacity">
                                    🌱
                                </span>
                            )}
                        </div>
