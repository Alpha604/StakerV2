...
import { Activity, TrendingUp, TrendingDown, Target, Coins, BarChart3, Star, Crown } from 'lucide-react';
...
export function Stats() {
  ...
  const nextVip = currentVipIdx < VIP_LEVELS.length - 1 ? VIP_LEVELS[currentVipIdx + 1] : null;
  const vipProgress = nextVip ? Math.min(100, Math.max(0, ((totalWageredAllTime - currentVip.req) / (nextVip.req - currentVip.req)) * 100)) : 100;
  ...
  if (!user || sessionBets.length === 0) {
    return (
       <div className="p-8 text-center text-text-secondary flex flex-col items-center justify-center min-h-[50vh]">
         ...
         <h2 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-accent to-blue-400 mb-4 inline-block">Statistiques de Session</h2>
         <p className="max-w-md">Jouez à des jeux pour voir vos statistiques s'afficher ici. L'historique et les données se mettront à jour en temps réel.</p>
         
         <div className="mt-12 bg-bg-panel border border-border-medium rounded-xl p-6 shadow-xl w-full max-w-lg text-left">
            <h3 className="text-white font-bold mb-4 flex items-center gap-2"><Crown size={20} className={currentVip.color} /> Niveau VIP Global</h3>
            ...
            <div className="h-4 w-full bg-bg-inner rounded-full overflow-hidden border border-border-subtle">
                <div className="h-full bg-accent transition-all duration-1000 ease-out" style={{ width: `${vipProgress}%` }}></div>
            </div>
            <p className="text-xs text-text-secondary mt-3">Total misé (tous les temps) : <span className="text-white font-mono">${totalWageredAllTime.toFixed(2)}</span></p>
         </div>
       </div>
    );
  }
    ...
    <div className="p-4 md:p-8 max-w-[1200px] mx-auto w-full flex flex-col gap-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2 pb-4 border-b border-border-medium">
         <div className="flex items-center gap-3">
             ...
             <h1 className="text-3xl font-black text-white tracking-tighter uppercase">Rapport de Session</h1>
         </div>
         <div className="flex items-center gap-4 bg-bg-panel px-4 py-2 rounded-lg border border-border-medium shadow-md">
             ...
             <div className="flex flex-col">
                 <span className="text-[10px] text-text-secondary uppercase font-bold tracking-widest">Niveau VIP</span>
                 <span className={cn("font-black leading-none", currentVip.color)}>{currentVip.name}</span>
             </div>
             <div className="w-[120px] ml-4">
                <div className="flex justify-between text-[10px] mb-1 font-bold">
                    <span className="text-text-secondary">{vipProgress.toFixed(1)}%</span>
                </div>
                <div className="h-1.5 w-full bg-bg-inner rounded-full overflow-hidden">
                    <div className="h-full bg-accent" style={{ width: `${vipProgress}%` }}></div>
                </div>
             </div>
         </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
         <div className="bg-bg-panel border border-border-medium rounded-xl p-6 shadow-xl flex flex-col gap-2 relative overflow-hidden transition-transform hover:-translate-y-1">
            ...
            <span className="text-text-secondary text-sm font-bold uppercase tracking-wider relative z-10">Mise Totale</span>
            <span className="text-3xl font-black text-white font-mono relative z-10">${totalWagered.toFixed(2)}</span>
            <span className="text-xs text-text-secondary mt-2 relative z-10">Dans la session actuelle</span>
         </div>
         <div className="bg-bg-panel border border-border-medium rounded-xl p-6 shadow-xl flex flex-col gap-2 relative overflow-hidden transition-transform hover:-translate-y-1">
            <div className="absolute top-0 right-0 p-4 opacity-5"><Target size={80}/></div>
            <span className="text-text-secondary text-sm font-bold uppercase tracking-wider relative z-10">Gains Totaux</span>
            <span className="text-3xl font-black text-white font-mono relative z-10">${totalPayout.toFixed(2)}</span>
            <span className="text-xs text-text-secondary mt-2 relative z-10">Paiements bruts</span>
         </div>
         <div className={cn("bg-bg-panel border rounded-xl p-6 shadow-xl flex flex-col gap-2 relative overflow-hidden transition-transform hover:-translate-y-1", totalProfit >= 0 ? "border-[#00e676]/50 shadow-[#00e676]/10" : "border-[#ed4163]/50 shadow-[#ed4163]/10")}>
            <div className="absolute top-0 right-0 p-4 opacity-10">
               {totalProfit >= 0 ? <TrendingUp size={80}/> : <TrendingDown size={80}/>}
            </div>
            <span className="text-text-secondary text-sm font-bold uppercase tracking-wider relative z-10">Bénéfice Net</span>
            <span className={cn("text-4xl font-black font-mono relative z-10", totalProfit >= 0 ? "text-[#00e676]" : "text-[#ed4163]")}>
               {totalProfit > 0 ? '+' : ''}{totalProfit.toFixed(2)}
            </span>
            <span className="text-xs text-white/50 mt-1 relative z-10">Taux de retour: {totalWagered > 0 ? ((totalPayout / totalWagered) * 100).toFixed(1) : 0}%</span>
         </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
         ...
         <div className="bg-bg-panel border border-border-medium rounded-xl p-6 shadow-xl lg:col-span-2">
            <div className="flex items-center gap-2 mb-6">
               ...
               <h3 className="text-white font-bold text-lg">Évolution du Bénéfice par Pari</h3>
            </div>
            <div className="h-[350px] w-full">
               <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={timeData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                     ...
                     <XAxis dataKey="name" stroke="#5b7b93" fontSize={12} tickLine={false} axisLine={false} dy={10} minTickGap={30} />
         ...
         {/* Pie Chart */}
         <div className="bg-bg-panel border border-border-medium rounded-xl p-6 shadow-xl flex flex-col">
            <h3 className="text-white font-bold mb-6 text-lg">Répartition des Paris</h3>
            <div className="h-[300px] w-full flex-1">
               <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                     <Pie
            ...
            <div className="flex flex-wrap justify-center gap-4 mt-6">
               {pieData.map((d, i) => (
                  <div key={d.name} className="flex items-center gap-2 text-sm font-bold text-text-secondary bg-bg-inner px-3 py-1.5 rounded-full border border-border-subtle hover:border-text-secondary transition-colors cursor-default">
                     <span className="w-2.5 h-2.5 rounded-full shadow-sm" style={{ backgroundColor: COLORS[i % COLORS.length] }}></span>
                     {d.name} <span className="text-white ml-1 bg-white/10 px-1.5 rounded text-xs">{d.value}</span>
                  </div>
               ))}
            </div>
         </div>
      ...
      <div className="bg-bg-panel border border-border-medium rounded-xl shadow-xl overflow-hidden mt-4">
         <div className="p-6 border-b border-border-medium bg-bg-inner/30 flex justify-between items-center">
            ...
            <span className="text-xs font-bold text-text-secondary bg-border-subtle px-2 py-1 rounded">{bets.length} PARIS</span>
         </div>
            ...
            <table className="w-full text-left border-collapse">
               ...
               <tbody>
                  {[...bets].reverse().map((b, i) => {
                     ...
                     return (
                        <tr key={b.timestamp + i + Math.random()} className="border-b border-border-medium/50 hover:bg-white/[0.04] transition-colors">
                           ...
                           <td className={cn("p-4 text-sm font-black font-mono text-right drop-shadow-sm", 
                               profit > 0 ? "text-[#00e676]" : profit < 0 ? "text-[#ed4163]" : "text-text-secondary")}>
